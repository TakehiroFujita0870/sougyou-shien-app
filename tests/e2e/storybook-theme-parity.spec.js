...
test('Storybook App stories receive the production global theme at 1440 × 900', async ({ page }) => {
  ...
  await page.goto('/');
  const applicationTheme = await page.locator('.kadode-shell').evaluate((element) => ({
  ...
  await page.goto('http://127.0.0.1:6007/iframe.html?id=kadode-app--home&viewMode=story');

  await expect(page.locator('#home-supervisor-message')).toBeVisible();
  const theme = await page.locator('.kadode-shell').first().evaluate((element) => ({
    canvas: getComputedStyle(element).getPropertyValue('--color-canvas').trim(),
    text: getComputedStyle(element).getPropertyValue('--color-text').trim(),
    font: getComputedStyle(element).fontFamily,
    background: getComputedStyle(element).backgroundColor,
  ...
  for (const id of ['kadode-app--project', 'kadode-app--knowledge', 'kadode-app--account']) {
    await page.goto(`http://127.0.0.1:6007/iframe.html?id=${id}&viewMode=story`);
    await expect(page.locator('.kadode-shell').first()).toBeVisible();
