...
test('records the desktop PII-free Home, Project, and Knowledge happy path', async ({ page }) => {
  ...
  const knowledgePreview = page.getByRole('dialog', { name: 'ナレッジに追加しますか？' })
  await expect(knowledgePreview).toContainText(knowledgeMessage)
  await knowledgePreview.getByRole('button', { name: 'キャンセル' }).click()
  await expect(page.getByRole('list', { name: 'ナレッジ一覧' })).not.toContainText(knowledgeMessage)
  await knowledgeComposer.fill(knowledgeMessage)
  await knowledgeComposer.press('Enter')
  await expect(knowledgePreview).toContainText(knowledgeMessage)
  await knowledgePreview.getByRole('button', { name: 'ナレッジに追加' }).click()
  await expect(page.getByRole('list', { name: 'ナレッジ一覧' })).toContainText(knowledgeMessage)
