...
| P5-01 | P5-SP-01 | 代表会話5件の検知→保存→preflight（自然会話試験） | 検査: 下記5件の合成会話マトリクスを実施し、各件について保存ID（無書込みはnone）、本文を含まないidempotency fingerprint、検索件数、fetch IDとprovenance、ResearchCampaign/ResearchRunの全体件数を記録する。保存・再送・新規ChatGPT会話からの再検索/取得を、同じ永続storeを再オープンして確認する。backend tool testsだけではモデルの会話検知・dispatchを証明できないため、明示的なconversation-to-tool dispatch seamを介したend-to-end試験、または本文を除いたredacted ChatGPT実会話証跡のいずれかを必須とする | 類推可能 |
...
| --- | --- | --- | --- | --- | --- |
| P5-01-A | 保存命令のない雑談調のアイデア共有。conversation-to-tool dispatch seamまたはredacted ChatGPT証跡で`capture_idea`への検知を確認 | 返却されたIdea IDを記録。初回と同じ要求の再送でSHA-256 fingerprint一致、`replayed=true`、target ID一致、重複作成0 | 保存後の固有語検索は1件 | 同じIdea IDをfetch。IdeaとSource/Revisionのprovenance参照が存在し、保存前のfixture IDへたどれる | 各0件（store全体の実数） |
| P5-01-B | 資料・資産候補を含む合成発話。実tool discoveryに`capture_source`がある場合だけ、許可された短い合成resource要約をSourceとして保存。`capture_asset`は呼ばず、Asset保存成功と扱わない | Source ID、SourceRevision ID、ContentChunk IDを記録。再送の入力SHA-256一致、`replayed=true`、各返却ID一致 | Source固有語検索は1件 | 返却されたSource IDをfetchし、Source→SourceRevision→ContentChunkのIDとprovenanceを照合。Asset IDはnone | 各0件（store全体の実数） |
| P5-01-C | 合成の協力候補PersonとOrganization。Personは既定`local_only`を維持し、外部検索0件・fetch拒否。Organizationは明示的に共有可能な合成項目のみ公開 | Person IDとOrganization IDを記録。両writeの入力SHA-256、再送時の`replayed=true`と同じIDを記録 | Person固有語検索0件、共有許可したOrganization固有語検索1件 | Organization IDをfetchしprovenanceを照合。Person fetchは拒否され、IDや非公開項目が返らない | 各0件（store全体の実数） |
...
同一接続内の連続tool testはこの受け入れを満たさない。各保存後にstdio/MCP接続を終了し、同じ永続storeを識別する非秘密のstore markerを照合してから新しいChatGPT会話・新しいstdio接続を開き、保存済みIDを会話へ渡さない検索とfetchを行う。backend tool testsはschema、保存、再送、projection、検索を検査するが、ChatGPT modelが自然会話を保存対象として検知したことは証明しない。この検知はconversation-to-tool dispatch seamのtrace、または会話本文・連絡先・credentialを除いたredacted ChatGPT証跡で別途立証する。いずれも用意できなければP5-01は未完了のままとする。
