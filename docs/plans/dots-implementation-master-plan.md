...
When: 「保存」または「Dotsへ書く」という語句を含まない日常的なアイデア発言をし、別の新規会話でその話題を尋ねる
...
| ID | Dependency | 成果物 | 完了判定（検査:） | 不確実性 |
| --- | --- | --- | --- | --- |
| P5-SP-01 | D-EXT-01,P3-03 | Secure MCP Tunnel tool discovery | 検査: 合成データだけで実際に公開されるsearch、fetch、capture_idea、capture_source、capture_person、capture_organizationをtool discoveryから記録する。2026-09-25時点の記録には`capture_asset`が含まれておらず、追加は後続のP5-SP-01Aで扱う | 検証済み（2026-09-25、当時のtool discovery記録を参照） |
| P5-SP-01A | P5-SP-01,P4-01 | Assetの用途限定MCP保存入口 | 検査: 合成metadata-only Assetをcreate-onlyで保存し、同一要求の再送で同じreceiptを返す。既定local_only、明示shareable時も名称・種別・安全な概要だけを検索・取得でき、本文・連絡先・秘密値を0件とする。memory / fake Neo4j / API / stdioのowner境界とsafe projectionを一致させ、tool discoveryへcapture_assetが現れることを確認する。外部取得、実データ、添付本文取込は行わない | 検証済み（2026-09-27、failing-first確認後にcapture_assetを追加。metadata-onlyの閉じたschema、default local_only、shareable allowlist、同一receipt replay、payload conflict、owner isolation、禁止field拒否をMCPで検査。fake Neo4j generic put parity、FastAPI、stdio/runtime discovery・annotation inventoryを検査。`uv run pytest backend/tests -q`: 917 passed, 6 skipped。実Neo4j・外部接続・実データは未使用） |
| P5-01 | P5-SP-01A | 代表5 fixtureの保存preflight（合成dispatch/storage受入試験） | 検査: 下記5件の固定合成fixtureを決定的なconversation-to-tool dispatch seamから実際のstdio MCPへ流し、使い捨てNeo4j永続storeのclose/reopenを挟んで各件の保存ID（無書込みはnone）、本文を含まないidempotency fingerprint、検索件数、fetch ID、内部provenanceとResearchCampaign/ResearchRunの全体件数を記録する。これはChatGPT modelの自然会話検知やresearch semantic gateを証明せず、redacted実会話試験は別途必要。実会話・ChatGPT接続・外部調査は使わない | 合成dispatch/storage preflightのみ実証。自然言語による保存対象の検知とresearch semantic gateは未検証 |
| P5-02 | P5-01 | ChatGPT接続runbookとredacted evidence | 検査: credential、会話原文、個人連絡先がartifactに0件になる | 類推可能 |
| P5-03 | P3-03,P5-SP-01A,P2-03,P5-04 | ChatGPT direct Graph RAG synthetic verification | 検査: 合成Idea / Asset / Person / Evidenceを保存・検索・取得する。人物→アイデアの関係は期限と根拠付きで保存し、ChatGPT検索から同じpathとEvidence IDを確認。Evidence取得は評価metadataだけとし、contact/private_notes/source textの露出を0件にする。write toolの共通receipt outputSchemaを通じ、返却target_idを手入力なしに次のtoolへ渡せることを試験する。Neo4j container再起動後に同じnode・pathを再検索し、代表会話5件とWindows再起動・再ログイン後の同一操作も確認する | 進行中（2026-09-25、通常Chatの一回の発話でIdea / Person / Claim / ADDRESSES / Evidence / 期限付きPerson→Idea関係を返却IDで連鎖保存し、検索とEvidence fetchまで実機確認。Evidence詳細のsafe projectionに本文・連絡先・private_notesなし。shareable Ideaの要約/snippetは検索応答へ含まれる。合成DBの永続volume再起動後は保存済みデータをMCP経由で再検索できたが、この再起動後のChatGPT UI再実行は未確認。代表会話5件、Windows実再起動・再ログインも未了） |
...
#### P5-01 代表5 fixtureの合成保存preflightマトリクス

すべて専用の空の合成永続storeと固有の検索語で実施する。各fixtureは識別子だけで管理し、会話本文、人物の実情報、資料本文は証跡へ残さない。idempotency evidenceは正規化したtool-call入力のSHA-256とwrite receiptのreplayed/target IDを記録し、入力値・idempotency key・原文は記録しない。各ケース終了時、store全体をauthoritative queryで数え、ResearchCampaign=0件かつResearchRun=0件を確認する。検索件数は専用検索語に対する全結果数である。

| Case | 合成シナリオと期待dispatch | 保存ID・再送fingerprint | 検索件数 | fetch ID・provenance | ResearchCampaign / ResearchRun |
| --- | --- | --- | --- | --- | --- |
| P5-01-A | 固定合成fixtureが`capture_idea`をdispatchし、保存・再送・storage preflightを検査する（自然言語検知の評価ではない） | 返却されたIdea IDとreceiptのSourceRevision IDを記録。初回と同じ要求の再送でSHA-256 fingerprint一致、`replayed=true`、target ID一致、重複作成0 | 保存後の固有語検索は1件 | 同じIdea IDをfetchし、MCPからprovenanceが隠れることを確認。再open後のowner-scoped内部読出しでIdea→SourceRevision→Source current_revision_idと各provenance target/source参照がreceipt IDに一致 | 各0件（store全体の実数） |
| P5-01-B | 合成の資料・資産候補。`capture_source`は既定`local_only`のまま保存・再送し、MCP search=0件/fetch拒否を確認する。Source→SourceRevision→ContentChunkのprovenanceは再open後のowner-scoped内部adapterで検査する。別途metadata-only `capture_asset`を明示shareableで保存し、AssetのみMCP検索1件/fetch成功を確認する | Source/SourceRevision/ContentChunk IDとAsset IDを記録。各writeの入力SHA-256、再送時の`replayed=true`と同じIDを記録 | Source固有語検索0件、Asset固有語検索1件 | MCP fetchは共有可能Assetのid/name/kind/description/statusだけを返し、Source fetchは拒否。provenanceは内部adapterのみで照合し、MCPへ出さない | 各0件（store全体の実数） |
| P5-01-C | 合成の協力候補PersonとOrganization。Personは既定`local_only`を維持し、検索結果に当該Personを含めずfetch拒否。Organizationは明示的に共有可能な合成項目のみ公開 | Person IDとOrganization IDを記録。両writeの入力SHA-256、再送時の`replayed=true`と同じIDを記録 | Person固有語検索に当該Person ID/nameなし（他の共有候補は件数に含む）、共有許可したOrganization固有語検索1件 | Organization IDをfetchし、再open後のowner-scoped内部読出しでshareable状態とprovenanceを確認。MCP projectionからprovenanceを除外。Person fetchは拒否され、IDや非公開項目が返らない | 各0件（store全体の実数） |
| P5-01-D | 既存の合成Ideaへ訂正と追加情報を与える。訂正は新しい履歴・provenanceとして保存し、過去版を上書きしない | 元Idea IDと訂正記録/新版IDを記録。訂正要求の入力SHA-256と再送receiptを記録し、再送で新しい版・記録が増えない | 現行版の固有語検索は1件。訂正前の内容は履歴参照を除き現行結果に出ない | 現行IDをfetchし訂正provenanceと旧版へのsupersedes/履歴参照を照合。元の保存IDも履歴で追跡可能 | 各0件（store全体の実数） |
| P5-01-E | 小話または仮定の検討で、記録・保存の意図がない対照会話。会話fixtureのhashだけを記録し、write dispatchなしを確認 | 保存ID=none、write fingerprint=none、write receipt=none。会話fixtureのSHA-256のみ記録し、tool call数0 | 専用固有語検索は0件 | fetch ID=none、provenance=none | 各0件（store全体の実数） |

同一接続内の連続tool testはこの受け入れを満たさない。各保存後にstdio/MCP接続とNeo4j driverを閉じ、同じ永続storeを識別する非秘密のstore markerを照合してから新しいstdio接続を開き、検索後の結果IDを使ってfetchする（訂正case Dのみ、訂正tool callに先行IDを渡す）。MCPのsafe projectionからprovenanceとIdeaの`supersedes_id`は除外し、owner-scoped内部adapterだけでprovenance・旧版参照を検査する。backend tool testsはschema、保存、再送、projection、検索を検査するが、ChatGPT modelが自然会話を保存対象として検知したことは証明しない。このdispatch試験は固定した合成fixtureからtool callを選ぶseamの永続化受け入れであり、自然会話検知は会話本文・連絡先・credentialを除いたredacted ChatGPT証跡で別途立証する。実会話検知証跡がなければ、その受け入れは未完了のままとする。
