...
| ID | Dependency | 成果物 | 完了判定（検査:） | 不確実性 |
| --- | --- | --- | --- | --- |
| P5-SP-01 | D-EXT-01,P3-03 | Secure MCP Tunnel tool discovery | 検査: 合成データだけで実際に公開されるsearch、fetch、capture_idea、capture_source、capture_person、capture_organizationをtool discoveryから記録する。`capture_asset`は公開実装が確認できないため想定・呼出し・実装しない | 検証済み（2026-09-25、tool discovery記録を参照） |
| P5-SP-01A | P5-SP-01,P4-01 | Assetの用途限定MCP保存入口 | 検査: 合成metadata-only Assetをcreate-onlyで保存し、同一要求の再送で同じreceiptを返す。既定local_only、明示shareable時も名称・種別・安全な概要だけを検索・取得でき、本文・連絡先・秘密値を0件とする。memory / Neo4j / stdioのowner境界とsafe projectionを一致させ、tool discoveryへcapture_assetが現れることを確認する。外部取得、実データ、添付本文取込は行わない | 類推可能 |
