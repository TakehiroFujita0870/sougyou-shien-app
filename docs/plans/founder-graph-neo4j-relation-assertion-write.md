...
| T-NRA-06 スパイク | #266 disposable Neo4j helperを再利用する読み取り専用gate設計 | 検査: synthetic-only fixture、T-NRA-03/04/05 transaction rollback注入点、既存migration、safe diagnostics、UUID対象cleanupを列挙。Docker起動なし | 未知（T-NRA-03〜05 merge後） |
| T-NRA-07 | 独立review/実行許可後の隔離Neo4j writer gate | 検査: 初回同一家族の競合insert・訂正・same-key replay・競合winner/conflict・late audit rollback/同key retryを実DBで検証し、owner/history/edges/audit/receiptを前後比較。UUID exact container/volume/network residue 0を記録。通常DB/service不使用 | 未知（T-NRA-06先行） |
