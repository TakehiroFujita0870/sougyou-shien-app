...
| Markdown表現 | IDと主要内容をJSON文字列として出力する。改行と引用符を決定的に表現し、raw HTMLとして解釈されない。 | 利用者入力をそのままMarkdown構文へ埋め込む案は表示注入を招くため却下。 | 実ダウンロード時も同じエスケープ規則を維持する。 |

## 変更履歴
| 日時 | 変更 | 理由 | 影響タスク |
| --- | --- | --- | --- |
| 2026-08-09 | 初版作成 | Issue #21をowner分離可能なlocal/fake契約へ分解 | T-1, T-2, T-3 |
| 2026-08-09 | Markdownに主要内容と決定的エスケープを追加 | Markdown単体で保管・移行できるようにする | T-1, T-2 |
