...
| GR-WR-02B | GR-WR-02A完了後に公開する用途限定capture_evidence MCP write | 検査: MCP / API / stdioの閉じた入力schemaからGR-WR-02Aだけを呼び、共通receiptを返す。ownerやexcerpt / locator / source textの入力を拒否し、tool discovery、再送、safe errorを確認する。既定はlocal_onlyとし、shareable Evidenceは同owner active shareable Claimと有効なローカルSourceRevision/ContentChunk lineageに限定する。Evidence projectionは自身のID・polarity・confidence・statusだけとし、claim/chunk/revision ID、本文・抜粋・locatorを除外する | 類推可能 |
...
| GR-WR-04 | assertionを通るsearch / fetchのsafe projection | 検査: currentなshareable RelationAssertionのみを経路に使い、status、confidence、valid_from、expires_at、path、shareable Evidence IDを保持する。Evidence→ContentChunk→SourceRevision→current Sourceのowner・active status・一意edgeと同owner active Claimを再検証する。local_only assertion / endpoint / Evidence、source text、locator、Evidence内のclaim/chunk/revision ID、内部履歴IDをMCP projectionから除外し、同点経路はassertion IDで決定する。旧Relationshipのlegacy `relation_path`形式とentity fetchの応答互換を維持し、直接assertion fetchはcurrent性を検証したsafe relation projectionを返す。in-memory/Neo4j parityをfocused testで確認する | 類推可能 |
| GR-WR-05 | stdioからNeo4j再起動後の統合保存・検索gate | 検査: 合成Idea、Claim、Evidence、RelationAssertionを実際のstdio JSON-RPC経路で保存し、同じ使い捨てNeo4j containerと同じvolumeをstop/startした後、stdio検索とfetchで同じID・経路を得る。shareable Evidenceは同owner active shareable Claimにのみ発行し、ローカルsource chainの本文はEvidence応答へ含めない。全JSON-RPC応答（content[].text、structuredContentを含む）でsource本文/URL/excerpt/locatorを除外し、search/fetch projectionではさらにsource/revision/chunkおよびEvidenceのClaim lineage IDを除外する。Shareable Claim endpoint IDは関係経路に必要な範囲で保持する。既知のsource-chain structural edgeを関係経路から除外し、不明なdomain edgeはfail-closedとする。UUIDで所有を確認できるcontainer、volume、networkだけを作成・再起動・cleanupする。実Neo4j再起動gate 1 passed、focused 134 passed / 1 skipped、全backend 913 passed / 6 skipped | 完了 |
...
| 2026-09-27 | GR-WR-04の受入条件を具体化。検索だけでなく直接assertion fetch、current性、source lineage、safe fields、同点時の決定性、legacy互換を明記 | 実装監査でmemory側のlocal_only assertion許可、Evidence lineage再検査不足、assertion fetchの意味未定義、同点候補の順序依存を確認したため | GR-WR-04 |
| 2026-09-27 | memory / Neo4jの検索と直接fetchでshareableかつcurrentなRelationAssertionのみを公開し、Evidence→Chunk→Revision→current SourceとClaimのowner・status・edge lineageをread時に再検査。同点候補はassertion IDで選択。direct fetchのMCP projectionはstatus/confidence/dates/path/Evidence IDに限定し、focused read 59 passed、全backend 904 passed / 5 skipped、py_compileとdiff check passed | GR-WR-04を既存legacy path互換を維持しつつ完了するため。Neo4j live instanceや実データは使わずfake-driverとin-memoryで検証した | GR-WR-04 |
| 2026-09-27 | GR-WR-05の受入条件を実stdio JSON-RPC、同一UUID所有container/volumeのstop/start、保存後search/fetchのsafe projectionまで具体化 | memory/fake-driverのテストだけでは実プロセス境界、Neo4j再起動後の永続性、応答artifactの秘匿を一括して証明できないため | GR-WR-05 |
| 2026-09-27 | GR-WR-05で判明したstdio経路の不足を反映し、capture_evidenceでshareable Claimに対するshareable Evidenceを限定許可 | 実Neo4j経路でshareable RelationAssertion保存が拒否された。Evidenceは本文を複製しないため、Evidence projectionを自身のID・polarity・confidence・statusに限定し、claim/chunk/revision IDsとsource chainはread projectionから除外する | GR-WR-02B、GR-WR-05 |
| 2026-09-27 | 実Neo4j search gateでknown source-chain structural edgeを除外し、unknown domain edgeのfail-closed検査を追加 | SourceからSourceRevision/Chunkへの内部edgeをRelationTypeとして誤検査してsearch全体が失敗したため | GR-WR-05 |
| 2026-09-27 | Neo4j ContentChunkにsource_revision_idをqueryable propertyとして書き、既存nodeでの欠落はpayload・edge・hash全体が一致する場合のみ互換読取 | 実source lineage gateではrevision referenceがpayloadにだけ存在し、queryable propertyの欠落によりread projectionを安全に検証できなかった。既存値の衝突は拒否しmigration不要とする | GR-WR-05 |
