...
- Idea predecessor lookupはpayload JSONをNeo4jで解釈せず、`_node_properties`が保存する`Idea.supersedes_id` scalar propertyと一致させる。strict typed decoderはpayload JSONを維持し、lineage queryはscalar propertyだけに依存する。rootには親IDpropertyを残さず、successorには親IDを文字列で保存する。
...
| T-IBP-02B | `founder_graph_neo4j.py`のIdea successor root/leaf lock、Brief lineage/latest CAS、atomic save/read/replay/audit、Run/Campaign historical proofと専用test | 検査: memory parity(initial/revision/get/latest/stale CAS/replay/fingerprint/created_at)、sorted root→leaf→Campaign lock、scalar Idea successor chain/current Brief、sibling fork no-mutation、global ID collision、Run/HAS_RUN/audit/history negative、rollback/unknown-commit recovery、focused31 passed、backend全suite、`git diff --check`を確認。02A/strict Run decoder merge後。 | 類推可能 |
...
| Idea predecessor property | `_node_properties`はIdeaのfull typed payloadに加えて非null`supersedes_id`をscalar propertyへ書き、child queryとstateful fakeはそのpropertyを照合する | fakeがpayload JSON内のreferenceを読んでNeo4j scalar predicateと違う挙動をする案、およびpayload-only persistenceでscalarを検索する契約を却下 | root→successor→current Brief lookupとsibling fork rejectionをNeo4jと同じ保存property契約でテストする |
...
| 2026-09-26 | PR #275 reviewで検出したIdea predecessor scalar-property/lineage-query不一致を修正。累計861変更行、root承認の900上限内 | fakeがpayload JSONを読む一方、Neo4j queryはscalar propertyを検索していた差を閉じ、root/current Brief/siblingを実データ形状と同じproperty契約で確認。focused31 passed、backend775 passed/4 skipped/7 warnings、diff-check clean | T-IBP-02B |
