...
| P4-05M0 | owner-scoped memory `IdeaBriefVersion` lineage storage/writer/readとfocused tests | 検査: same-owner current Idea/root lineageとimmutable Idea revision IDを照合し、brief revision/supersedes連続性、必須expected-latest CAS、同key同intent replay/異intent conflict、authoritative latest lookup、audit fault rollbackを検査。nonempty `research_run_ids`はmemory内のtyped completed Run、exact `HAS_RUN`、write receipt/audit、typed Campaign historyを解決してP4-05V-Mを通す。service内の凍結typed valuesだけ保持し、caller status/latest claimsを信用しない。差分500行以内 | 既知（実装済み、focused/full backend tests pass、独立review待ち） |
| P4-05M1 | concrete-only `InMemoryGraphWriteService.save_relation_assertion`とtests | 検査: owner/type/current endpointとactive Evidence refs、assertion↔typed endpoints/Evidence/successor structural edges、audit/replay/no-mutation/rollback/family historyを検査。Idea関係はM0からexact latest Briefと正確なsectionを解決し、Idea endpointとの一致、全8 section、service内registered completed Run・exact `HAS_RUN`・receipt・Campaign typed historyを照合してP4-05V-Mを呼ぶ。post-Run expiry/revokeは遡及適用しない。`GraphWritePort`は変更せず、Neo adapterが同じ契約を実装するまで共有Protocol/APIへ追加しない。差分500行以内 | 未知（M0・V-M後、owner release後） |
...
| structural assertion refs | P4-05M1でcanonical assertion-edge labelsとtyped endpoint pairsをdomain contractとして追加・検査する | main `RelationType`にはAssertionの端点/evidence edgeが未定義で、未知edgeを暗黙にlistへ追加するとread/query parityがない | Neo transaction packetと同じlabel/direction/type invariantを要求 |
