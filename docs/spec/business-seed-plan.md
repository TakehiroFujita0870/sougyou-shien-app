...
> 2026-09-20以降、以下の5観点は既存入力契約として保持し、事業評価レポートの章名には使わない。新しいレポートの章順、名称、版管理は[Dots Founder Graph ピボット・実装全体計画](../plans/founder-graph-pivot.md#事業評価レポート契約)を正本とする。
...
- [PRD v0.2](PRD.md)はピボット前の製品体験とMVP受入基準である。本計画はその「アイデア仮説カード」を事業のタネへ具体化した履歴を残す。
- [横断調査・個人ナレッジ・意思決定記憶計画](research-memory-plan.md)のEvidence、削除伝播、過去判断を移植候補として参照する。
- [仮説検証・調査支援仕様](falsification-engine.md)の「市場の見込み」「競合との違い」「攻めどころの特定」は、ピボット後のClaim / Evidenceへ移植する入力である。
- 競合との違いと攻めどころは、競合ページの勝ち筋・機会として保存する。市場ページと競合ページのどちらからも更新できるが、同じEvidence IDを重複保存しない。

## 事業評価レポートの章立て

レポート出力は次の8章に固定する。旧案の「Founder」を含む章名は使わない。
...
既存の「どんな事業」「市場」「競合」「利益」「実現性」は、移行中の入力データを失わないためのsource viewとして扱う。新しいReportVersionでは、同じClaimとEvidenceを8章へ投影し、章ごとに複製しない。
...
| 競合は誰？ | 3種競合と勝ち筋 | 直接競合、間接競合、代替手段、比較軸、潜在参入リスク | 勝ち筋、機会、反証、本人判断 |
| 利益はでる？ | 損益分岐の前提と状態 | unit economics、値付け、変動費、固定費、損益分岐、予測PL・BS・CF | 計算根拠、シナリオ、本人判断 |
| 実現できる？ | 資金・resource・両立の制約 | 銀行・公庫候補、必要resource、本業・家庭制約、可逆な次の検証、停止条件 | 実行計画、依存、本人判断 |

visual conceptは、事業を説明する画像の目的、対象、構図、禁止表現、画像prompt案を記録する。画像生成や外部送信は行わず、promptは本人所有の機微情報として扱う。

3種競合は、同じ顧客へ類似した提供物を売る`直接競合`、異なる提供物で同じ予算・目的を取り合う`間接競合`、内製・手作業・何もしない選択を含む`代替手段`とする。潜在参入者は3種へ混ぜず、将来リスクとして別に記録する。勝ち筋と機会は競合情報そのものではなく、Evidenceを参照する本人判断として保存する。

### 利益ページの計算契約

- `売上 = 単価 × 販売数量`、`限界利益 = 売上 − 変動費`、`限界利益率 = 限界利益 ÷ 売上`、`損益分岐売上 = 固定費 ÷ 限界利益率`を基本式とする。
- unit economicsは1顧客または1販売単位を明示し、単位当たり売上、単位当たり変動費、単位当たり限界利益を表示する。
- 全入力に対象期間、通貨、税込・税抜、丸め規則、仮説・実績の区分を持たせる。売上または限界利益率が0以下なら損益分岐を未計算とし、理由を表示する。
- 予測PLは売上、変動費、限界利益、固定費、営業利益、予測BSは現金、その他資産、借入金、その他負債、純資産、予測CFは営業・投資・財務CFと現金増減を必須行とする。
- 各シナリオは開始残高、自己資金、借入を入力し、`資産 = 負債 + 純資産`と`期末現金 = 期首現金 + 現金増減`の不一致をエラー表示する。税・減価償却の初回範囲はQ-60-02で決め、未決の行を0円と仮定しない。
...
Then: 根拠、AI推論、本人判断が区別され、直接競合、間接競合、代替手段の未入力と潜在参入リスクが表示され、勝ち筋と機会は本人判断として別に保存される
...
Given: 開始残高、販売数量、単価、変動費、固定費、自己資金、借入を含む予測シナリオがある

When: 利用者が予測PL・BS・CFを計算する

Then: 基本式、3表の必須行、損益分岐、貸借一致、現金繰越を検証し、0除算、不一致、未決の税・減価償却は未計算理由として表示される
...
Given: 週当たり投入可能時間、自己資金上限、外部承認依存、不可逆な行為を本人が記録している

When: 次の実行計画を保存する

Then: 次の可逆な検証、必要resource、停止条件が本人判断として表示され、時間・資金上限を超える行為と契約・申請・送信は自動実行されない

### US-60-06 事業計画書を取得する

As a 事業案を説明する利用者, I want 各観点から事業計画書をexportしたい, so that 現時点の根拠と未確認事項を持ち出して見直せる。

Given: 本人が5観点のいずれかのページを開いている

When: export対象と含める個人制約を確認して実行する

Then: 起点ページにかかわらず選択済みの本人所有データだけを含むローカルファイルが生成され、未入力観点は空欄・未確認と表示され、外部サービスへ送信されない
...
Then: 本人所有でknowledge単位に同意済みの記録だけが候補になり、参照元・参照先project、根拠、許可状態が表示され、顧客ヒアリング由来は匿名化済み派生物だけが使われる
...
| 顧客ヒアリング | 本人。市場ページと匿名化済みの横断knowledge | 第三者の氏名、連絡先、識別子は横断候補・外部送信に使わない | 原記録と匿名化派生物を削除し検索・既存参照から除外 |
| 財務仮説 | 本人。利益ページ | 通貨、期間、仮説か実績かを表示 | 個別削除でき、選択しなければexportしない |
| 画像prompt | 本人。どんな事業ページ | 外部画像生成をしないことを表示 | 個別削除でき、exportは明示選択のみ |
| project横断knowledge | 本人。knowledge単位で許可した送信元・送信先project間だけ | 参照元・参照先と許可日時を表示 | 許可取消で候補・検索・exportから除外し、元データ削除で既存参照も利用不可 |

ピボット前の全所有データは、`owner_id`、RLS、認証由来の所有者確定、削除時の原本・抽出本文・埋め込み除外を[research-memory-plan.md](research-memory-plan.md)で定義した。この境界はPhase 0の移植候補であり、新規実装はFounder Graph計画の単独利用、egress policy、provenanceを優先する。

P0の横断許可は`knowledge_grants`にowner_id、knowledge_id、source_project_id、target_project_id、granted_at、revoked_atを持つknowledge単位の明示許可とする。許可取消または元データ削除後は新規候補、検索、exportから直ちに除外し、既存レポートには参照不可と表示する。顧客ヒアリング由来のknowledgeは匿名化済み派生物だけにgrantを作成し、原記録へのgrantを拒否する。
...
| P0 | 5観点で根拠、AI推論、本人判断を分け、直接・間接・代替の3種競合、勝ち筋、機会、財務基本式と予測3表を表示する |
...
2. 本人が競合ページから直接競合、間接競合、代替手段、潜在参入リスクを記録し、根拠と勝ち筋を本人判断として保存する。
3. 本人が利益ページで価格と費用の仮説を入力し、基本式、損益分岐、予測PL・BS・CFの整合を確認する。
4. 本人が実現性ページで銀行・公庫候補、本業・家庭制約、時間・自己資金上限を入力し、可逆な次の検証と停止条件を保存する。
5. 本人が匿名化済みknowledgeをknowledge単位で別projectへ許可し、取消・原本削除後に検索、既存参照、exportから利用不能になることを確認する。
6. 5観点の各ページを起点にexportし、未入力観点、選択外の機微情報、外部リクエスト0件をパラメータ化E2Eで確認する。
7. 別利用者が同じURLまたはIDを試しても、事業のタネ、5観点、export、横断knowledgeの全てで404または0件となる。
...
| Q-60-05 | P1で複数knowledgeの一括許可を追加する場合、誤共有を防ぐ確認画面をどう設計するか | CEO | P1横断knowledge計画前 |
...
| SP-60-03 | 顧客ヒアリング匿名化の品質評価 | 検査: 合成データで識別子除去、原記録grant拒否、削除、許可取消、他owner拒否を記録し1営業日で終了 | 未知・先行スパイク |
...
| T-60-04 | 競合ページの3種分類、勝ち筋、機会の表示 | 検査: US-60-03の直接・間接・代替と潜在参入リスク、根拠・推論・本人判断のUI/APIテストが通る | research-memory-plan.mdのT-03後に類推可能 |
| T-60-05 | 利益ページの計算契約と予測財務表示 | 検査: 基本式、0除算、単位・期間・通貨・税区分、予測PL・BS・CF、貸借・現金繰越のテストが通る | SP-60-02とQ-60-02決定後に類推可能 |
| T-60-06 | 実現性ページ、データ同意、project横断knowledgeの所有者契約 | 検査: US-60-05、US-60-07、時間・資金上限、匿名化済み派生物、別owner拒否、削除・許可取消のテストが通る | SP-60-03とresearch-memory-plan.mdのT-01後に類推可能 |
| T-60-07 | ローカル事業計画書export | 検査: 5観点の各ページを起点に、未入力表示、選択外機微情報なし、外部リクエスト0件のパラメータ化E2Eが通る | SP-60-01後に類推可能 |
...
| ADR-60-02 競合の分類 | 直接競合、間接競合、代替手段の3種を保存し、潜在参入者は将来リスクに分ける。現在の比較対象と将来の脅威を区別できる | 競合を一つの一覧にまとめる案は、比較対象とリスクの意味が混ざるため却下 | 勝ち筋と機会は本人判断として別記録にする |
| ADR-60-03 財務の扱い | 基本式、単位、期間、通貨、税区分、仮説状態、予測PL・BS・CFの整合を保存する。推計の再現性と資金繰りを示せる | AIが利益や融資可能性を確定値として提示する案は、誤認を招くため却下 | 実績・外部接続はCEO決裁後に再評価 |
| ADR-60-04 機微データ | 銀行名、顧客ヒアリング、財務、画像prompt、個人制約は本人所有・明示選択・個別削除とし、横断knowledgeはknowledge単位で許可する | project間の既定共有と外部サービスへの自動送信は、同意と最小権限を満たさないため却下 | ピボット前はresearch-memory-plan.mdのRLSと削除契約へ従属。新規実装はFounder Graph計画へ移植する |
...
| 横断knowledgeの無断利用 | project間の期待違反 | knowledge単位の明示許可、参照元・参照先表示、許可取消 |
...
| 2026-09-20 | 5観点を移植元へ変更し、事業評価レポートの8章を明記 | Founder Graphピボットと指定章名を反映するため | T-FG-18、T-FG-25、T-FG-26 |
