...
2. Build `idempotency_key` as `repository + issue_or_pr + head_or_merge_sha + event + target`.
...
5. Keep the sender incomplete until receipt is confirmed. On delivery failure or timeout, return `HANDOFF_FAILED` with target, event, idempotency key, and retry state.
...
9. In the final response, name the accepted target, event, and idempotency key. Do not report “complete” or “waiting” before receipt.
