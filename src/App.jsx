...
export function App({ profileRepository, adoptedProjectRepository, homeConversationRepository, sidebarPortfolioRepository }) {
  ...
  function workspaceContent() {
    ...
    if (activeWorkspace === 'knowledge') return <KnowledgeSurface fixture={knowledgeDemoFixture} profile={profileHydration.value} ownerId="local-owner" spaceId="local-space" availableProjects={availableProjects} archiveHistory={[...portfolio.home, ...portfolio.project].filter((item) => item.archived)} onOpenProject={async (projectId, evaluationView) => { const project = availableProjects.find((item) => item.id === projectId && item.ownerId === 'local-owner' && item.spaceId === 'local-space'); if (!project || !projectEvaluationTabs.includes(evaluationView)) return; adoptedProjectHydration.replaceReady(project); setProjectTargetView(evaluationView); setActiveWorkspace('project'); }} onAssetAdded={async (asset) => { const next = await portfolioRepositoryRef.current.ensure('knowledge', { id: asset.id, title: asset.name, unread: true, updatedAt: Date.now() }); setPortfolio(next); setPortfolioError(''); }} onSend={(value) => { void portfolioRepositoryRef.current.ensure('knowledge', { id: portfolioMessageId('knowledge:conversation', value), title: value.slice(0, 80), unread: true, updatedAt: Date.now() }).then((next) => { setPortfolio(next); setPortfolioError(''); }).catch(() => setPortfolioError('履歴を保存できませんでした。もう一度お試しください。')); }} modelKey={homeModelKey} models={getHomeModels()} onModelChange={updateModel} />;
