...
export function FileLibrary({ repository = createFakeDocumentRepository(), initialDocuments = [] }) {
  ...
  return <section className="w-full max-w-3xl rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-7" aria-labelledby="files-heading">
    ...
    <ul className="mt-6 grid gap-3" aria-label="登録済み資料">{documents.filter((document) => document.state !== 'deleted').map((document) => <li key={document.id} className="rounded-2xl border border-stone-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><strong>{document.name}</strong><p className="mt-1 text-sm text-stone-600">v{document.version} · {stateLabel[document.state]}</p>{document.references?.filter((reference) => reference.status !== 'unavailable').length > 0 && <ul className="mt-3 grid gap-1 text-sm text-stone-600" aria-label={`${document.name}の出典`}><li className="font-bold text-stone-700">space内の出典</li>{document.references.filter((reference) => reference.status !== 'unavailable').map((reference) => <li key={`${reference.kind}-${reference.sourceId}-${reference.locator}`}>{reference.kind}: {reference.sourceId} · {reference.locator}</li>)}</ul>}</div><div className="flex flex-wrap gap-2">{document.state === 'failed' && <button type="button" onClick={async () => update(await repository.retry(document.id))} className="rounded-full border px-3 py-2 text-sm font-bold">再試行</button>}<label className="cursor-pointer rounded-full border px-3 py-2 text-sm font-bold">新版<input className="sr-only" type="file" accept=".pdf,.docx,.txt,.csv" onChange={(event) => event.target.files[0] && upload(event.target.files[0], document)} /></label><button type="button" onClick={() => setDeleteTarget(document)} className="rounded-full border border-red-200 px-3 py-2 text-sm font-bold text-red-800">削除</button></div></div></li>)}</ul>
