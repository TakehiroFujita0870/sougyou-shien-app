...
const dateText = (value) => {
  ...
  return Number.isFinite(date.getTime()) ? new Intl.DateTimeFormat('ja-JP', { month: 'short', day: 'numeric' }).format(date) : '日付未確認';
};
const sourceLabel = { synthetic: '画面生成', local: 'この端末', unknown: '出典未確認' };
const confidenceLabel = { high: '高', medium: '中', unknown: '未確認' };

export function KnowledgeSurface({ fixture, repository, conversationRepository, profile, archiveHistory = [], ownerId = 'admin-demo-owner', spaceId = 'admin-demo-space', onSend = () => {}, onAssetAdded = () => {}, onOpenProject = () => {}, modelKey = 'gpt-5.6-terra', models = [], onModelChange }) {
  const [message, setMessage] = useState(''); const [documents, setDocuments] = useState([]); const [state, setState] = useState({ messages: [], entries: [] });
  const [candidate, setCandidate] = useState(null); const [category, setCategory] = useState('all'); const [query, setQuery] = useState(''); const [removeRequested, setRemoveRequested] = useState(null); const [notice, setNotice] = useState(''); const [writeError, setWriteError] = useState(''); const [writePending, setWritePending] = useState(false);
  ...
  const items = useMemo(() => {
    const fixtureFile = fixture?.asset ? [{ id: fixture.asset.id, category: 'file', title: fixture.asset.name, content: 'プロジェクトに紐づく参照資料', createdAt: fixture.asset.updatedAt ?? new Date().toISOString(), state: fixture.asset.state, sourceType: 'synthetic', confidence: 'unknown', unknowns: [] }] : [];
    const files = documents.map((file) => ({ id: file.id, category: 'file', title: file.name, content: `${file.mediaType?.toUpperCase() ?? '資料'} · ${Math.max(1, Math.ceil((file.sizeBytes ?? 0) / 1024))} KB`, createdAt: new Date(file.lastModified ?? 0).toISOString(), state: file.state, sourceType: 'local', confidence: 'unknown', unknowns: [] }));
    const decision = fixture?.decision ? [{ id: `decision:${fixture.decision.id ?? 'current'}`, category: 'decision', title: fixture.decision.judgement, content: `${fixture.decision.background} ${fixture.decision.reason}`, createdAt: fixture.decision.date ?? '2026-01-01T00:00:00.000Z', sourceType: 'unknown', confidence: 'unknown', unknowns: [] }] : [];
    const profileEntry = profile?.status === 'completed' ? [{ id: 'profile:completed', category: 'profile', title: '完了プロフィール', content: Object.values(profile.values ?? {}).filter((value) => typeof value === 'string' && value.trim()).join(' · '), createdAt: profile.completedAt ?? profile.updatedAt ?? '2026-01-01T00:00:00.000Z', sourceType: 'unknown', confidence: 'unknown', unknowns: [] }] : [];
    const conversations = state.messages.filter((entry) => entry.role === 'user').map((entry, index) => ({ id: `conversation:${index}:${entry.content}`, category: 'conversation', title: entry.content.slice(0, 72), content: 'Kadode AIとの過去の会話', createdAt: entry.createdAt ?? '1970-01-01T00:00:00.000Z', sourceType: 'unknown', confidence: 'unknown', unknowns: [] }));
  ...
  async function confirmRemoval() { const target = removeRequested; if (!target || writePending) return; setWritePending(true); setWriteError(''); try { await repositoryRef.current.delete(target.id, target); setDocuments(repositoryRef.current.list().filter((item) => item.state !== 'deleted')); setRemoveRequested(null); setNotice('ファイルを一覧から削除しました。'); } catch { setWriteError('ファイルを削除できませんでした。もう一度お試しください。'); } finally { setWritePending(false); } }
  return <section aria-labelledby="knowledge-heading" className="kadode-composer-layout mx-auto grid min-h-[calc(100vh-3rem)] w-full max-w-6xl gap-5 pb-36">
    ...
    {notice && <p role="status" className="text-sm text-[var(--color-text-muted)]">{notice}</p>}{writeError && <p role="alert" className="text-sm text-red-700">{writeError}</p>}
    ...
    {archiveHistory.length > 0 && <h2 className="text-sm font-semibold text-[var(--color-text-muted)]">アーカイブ履歴</h2>}<ol aria-label="ナレッジ一覧" className="grid gap-2">{items.length ? items.map((item) => <li key={item.id}><Card className="flex items-start gap-3 p-3"><Badge variant="outline">{categoryLabel[item.category]}</Badge><div className="min-w-0 flex-1"><h2 className="truncate text-sm font-semibold">{item.title}</h2><p className="mt-1 line-clamp-1 text-xs text-[var(--color-text-muted)]">{item.content}</p><div className="mt-2 flex flex-wrap items-center gap-1 text-xs text-[var(--color-text-muted)]"><Badge variant="outline">{sourceLabel[item.sourceType] ?? sourceLabel.unknown}</Badge><Badge variant="outline">確度: {confidenceLabel[item.confidence] ?? confidenceLabel.unknown}</Badge>{item.projectId && <Button type="button" variant="ghost" className="min-h-11 px-3 text-xs" onClick={() => onOpenProject()}>Projectを開く</Button>}</div></div><time className="shrink-0 text-xs text-[var(--color-text-muted)]">{dateText(item.updatedAt)}</time>{item.category === 'file' && item.id.startsWith('local-file:') && <Button type="button" variant="ghost" className="min-h-8 px-2 text-xs" onClick={() => setRemoveRequested(item)}>削除</Button>}</Card></li>) : <li><Card className="p-5 text-sm text-[var(--color-text-muted)]">条件に合うナレッジはまだありません。</Card></li>}</ol>
    <Dialog open={Boolean(candidate)} onOpenChange={(open) => !open && setCandidate(null)}><DialogContent><DialogTitle>ナレッジに追加しますか？</DialogTitle>{candidate && <><DialogDescription className="mt-2">AIが分類とタイトル案を作成しました。保存前に確認できます。</DialogDescription><p className="mt-4 text-sm"><Badge variant="outline">{categoryLabel[candidate.category]}</Badge></p><p className="mt-2 font-semibold">{candidate.title}</p><p className="mt-2 text-sm text-[var(--color-text-muted)]">{candidate.content}</p></>}<div className="mt-5 flex gap-2"><Button disabled={writePending} onClick={() => void confirmCandidate()}>{writePending ? '保存中…' : 'ナレッジに追加'}</Button><DialogClose asChild><Button disabled={writePending} variant="secondary">キャンセル</Button></DialogClose></div></DialogContent></Dialog>
    <Dialog open={Boolean(removeRequested)} onOpenChange={(open) => !open && setRemoveRequested(null)}><DialogContent><DialogTitle>このファイルを削除しますか？</DialogTitle><DialogDescription>ファイル名と端末内メタデータを一覧から削除します。</DialogDescription><div className="mt-5 flex gap-2"><Button disabled={writePending} onClick={() => void confirmRemoval()}>{writePending ? '削除中…' : '削除を確定'}</Button><DialogClose asChild><Button disabled={writePending} variant="secondary">キャンセル</Button></DialogClose></div></DialogContent></Dialog>
    <AiComposer id="knowledge-composer" label="KnowledgeについてKadode AIに相談" value={message} onValueChange={setMessage} onSubmit={submit} mode="anchored" className="mx-auto w-full max-w-4xl" textareaClassName="kadode-composer__textarea--compact" placeholder="追加したい情報はありますか？" modelKey={modelKey} models={models} onModelChange={onModelChange} sendAriaLabel="Knowledgeの質問を送信" sendIcon={<SendHorizontal className="size-4" />} />
  </section>;
}
