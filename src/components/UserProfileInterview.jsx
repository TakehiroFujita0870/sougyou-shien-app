...
function createInterviewState(profile) {
  if (!profile) return { values: EMPTY_PROFILE, step: 0, status: 'in_progress', error: '' };
  return { ...profile, values: { ...EMPTY_PROFILE, ...profile.values }, status: profile.status === 'completed' ? 'completed' : 'in_progress', error: '' };
}

export function UserProfileInterview({ initialProfile = null, repository, onClose, onComplete }) {
  const [state, setState] = useState(() => createInterviewState(initialProfile));
  ...
  async function save(next) { setSaveError(''); const result = await persistProfile(repository, next); if (result.error) return setSaveError(result.error); setState(result.profile); if (result.profile.status === 'completed') onComplete?.(result.profile); }
  function submit(event) { event.preventDefault(); const next = advanceProfile(state); if (next.error) return setState(next); save(next); }
  if (state.status === 'completed') return <section className="Dots-dialog-panel w-full min-w-0 max-w-full overflow-hidden rounded-3xl p-6 shadow-xl sm:max-w-2xl" aria-labelledby="profile-heading"><div className="flex min-w-0 justify-between gap-4"><div className="min-w-0"><p className="Dots-dialog-kicker font-bold">あなたの情報</p><h2 id="profile-heading" className="break-words text-2xl font-bold">プロフィールを更新</h2></div><button className="shrink-0" onClick={onClose} type="button" aria-label="ヒアリングを閉じる">閉じる</button></div><p className="Dots-dialog-complete mt-5 break-words rounded-2xl p-4">入力内容は保存されています。変更したい項目を選んで更新できます。</p><div className="mt-4 grid min-w-0 gap-2">{PROFILE_STEPS.map(([key, label], step) => <button key={key} type="button" onClick={() => setState({ ...state, step, status: 'in_progress' })} className="min-w-0 rounded-xl border p-3 text-left"><strong className="break-words">{label}</strong><span className="block break-words text-sm">{state.values[key]}</span></button>)}</div></section>;
  return <section className="Dots-dialog-panel w-full min-w-0 max-w-full overflow-hidden rounded-3xl p-6 shadow-xl sm:max-w-2xl" aria-labelledby="profile-heading"><div className="flex min-w-0 justify-between gap-4"><div className="min-w-0"><p className="Dots-dialog-kicker font-bold">あなたの情報</p><h2 id="profile-heading" className="break-words text-2xl font-bold">一緒に整理していきましょう</h2></div><button className="shrink-0" onClick={onClose} type="button" aria-label="ヒアリングを閉じる">閉じる</button></div><form className="mt-6 min-w-0" onSubmit={submit} noValidate><p className="Dots-dialog-progress text-sm font-bold">{state.step + 1} / {PROFILE_STEPS.length}</p><p id="profile-question" className="mt-3 break-words text-lg font-bold">{question}</p><textarea autoFocus value={state.values[field]} onChange={(event) => setState({ ...state, values: { ...state.values, [field]: event.target.value }, error: '' })} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submit(event); } }} rows={4} maxLength={300} aria-invalid={Boolean(state.error)} aria-labelledby="profile-question" aria-describedby={state.error ? 'profile-error' : 'profile-help'} placeholder={placeholder} className="mt-3 w-full min-w-0 max-w-full rounded-2xl border p-4"/><p id="profile-help" className="mt-2 break-words text-xs">Enterで送信、Shift+Enterで改行。Escで閉じます。</p>{state.error && <p id="profile-error" role="alert" className="Dots-error mt-2 break-words">{state.error}</p>}{saveError && <p role="alert" className="Dots-error mt-2 break-words">{saveError}</p>}<div className="mt-5 flex min-w-0 flex-wrap gap-3"><button type="submit" className="Dots-dialog-submit rounded-full px-5 py-3 font-bold">{state.step === PROFILE_STEPS.length - 1 ? '保存して完了' : '送信して次へ'}</button>{state.step > 0 && <button type="button" onClick={() => setState({ ...state, step: state.step - 1, error: '' })}>戻る</button>}</div></form></section>;
}
