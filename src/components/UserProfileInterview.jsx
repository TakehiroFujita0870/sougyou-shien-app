...
export function UserProfileInterview({ repository = createBrowserProfileRepository(), onClose, onComplete }) {
  const [state, setState] = useState({ values: EMPTY_PROFILE, step: 0, status: 'loading', error: '' });
  const [saveError, setSaveError] = useState('');
  useEffect(() => { repository.load().then((saved) => setState(saved ? { ...saved, status: saved.status === 'completed' ? 'completed' : 'in_progress', error: '' } : { values: EMPTY_PROFILE, step: 0, status: 'in_progress', error: '' })).catch(() => setState({ values: EMPTY_PROFILE, step: 0, status: 'in_progress', error: '' })); }, [repository]);
  ...
  async function save(next) { setSaveError(''); const result = await persistProfile(repository, next); if (result.error) return setSaveError(result.error); setState(result.profile); if (result.profile.status === 'completed') onComplete?.(result.profile.values); }
  ...
  return <section className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-xl" aria-labelledby="profile-heading"><div className="flex justify-between gap-4"><div><p className="font-bold text-emerald-700">あなたの情報</p><h2 id="profile-heading" className="text-2xl font-bold">一緒に整理していきましょう</h2></div><button onClick={onClose} type="button" aria-label="ヒアリングを閉じる">閉じる</button></div><form className="mt-6" onSubmit={submit} noValidate><p className="text-sm font-bold text-emerald-800">{state.step + 1} / {PROFILE_STEPS.length}</p><p className="mt-3 text-lg font-bold">{question}</p><textarea autoFocus value={state.values[field]} onChange={(event) => setState({ ...state, values: { ...state.values, [field]: event.target.value }, error: '' })} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submit(event); } }} rows={4} maxLength={300} aria-invalid={Boolean(state.error)} aria-describedby={state.error ? 'profile-error' : 'profile-help'} placeholder={placeholder} className="mt-3 w-full rounded-2xl border p-4"/><p id="profile-help" className="mt-2 text-xs">Enterで送信、Shift+Enterで改行。Escで閉じます。</p>{state.error && <p id="profile-error" role="alert" className="mt-2 text-red-700">{state.error}</p>}{saveError && <p role="alert" className="mt-2 text-red-700">{saveError}</p>}<div className="mt-5 flex gap-3"><button type="submit" className="rounded-full bg-emerald-800 px-5 py-3 font-bold text-white">{state.step === PROFILE_STEPS.length - 1 ? '保存して完了' : '送信して次へ'}</button>{state.step > 0 && <button type="button" onClick={() => setState({ ...state, step: state.step - 1, error: '' })}>戻る</button>}</div></form></section>;
}
