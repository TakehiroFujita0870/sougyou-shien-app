...
export function AiComposer({
  ...
  const content = <><label htmlFor={id} className="sr-only">{label}</label><textarea id={id} disabled={textareaDisabled} value={value} onChange={(event) => onValueChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) submit(event); }} rows={rows} maxLength={maxLength} className={`kadode-composer__textarea ${textareaClassName}`.trim()} placeholder={placeholder} />{children}<div className={`kadode-composer__actions ${actionsClassName}`.trim()}>{leadingActions}{groupTrailingActions ? <div className="flex items-center gap-1">{trailing}</div> : trailing}</div></>;
