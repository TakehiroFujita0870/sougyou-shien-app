...
export function AiComposer({
  ...
  const content = <><label htmlFor={id} className="sr-only">{label}</label><textarea id={id} disabled={textareaDisabled} value={value} onChange={(event) => onValueChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) submit(event); }} rows={rows} maxLength={maxLength} className={`Dots-composer__textarea ${textareaClassName}`.trim()} placeholder={placeholder} />{children}<div className={`Dots-composer__actions ${actionsClassName}`.trim()}>{leadingActions}{groupTrailingActions ? <div className="flex items-center gap-1">{trailing}</div> : trailing}</div></>;
  const surfaceClass = `Dots-composer ${mode === 'anchored' ? 'Dots-composer--anchored' : ''} ${className}`.trim();
