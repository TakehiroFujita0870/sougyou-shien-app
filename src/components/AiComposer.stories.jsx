...
function ComposerStory({ mode = 'inline', compact = false, project = false }) {
  ...
  return <div className="min-h-[420px] p-6"><AiComposer id="storybook-ai-composer" label="Dots. AIへのメッセージ" value={value} onValueChange={setValue} onSubmit={() => setValue('')} placeholder="事業について相談する" mode={mode} modelKey={modelKey} models={models} onModelChange={setModelKey} textareaClassName={compact ? 'Dots-composer__textarea--compact resize-none' : 'min-h-36 resize-none'} groupTrailingActions={project} leadingActions={project ? <Button type="button" variant="ghost" className="min-h-9 gap-1 px-2 text-xs"><Sparkles className="size-4" />AIで補完</Button> : <Button type="button" variant="ghost" disabled aria-label="音声入力（準備中）"><Mic className="size-4" /></Button>} /></div>;
}

export default { title: 'Dots./AiComposer', component: AiComposer, parameters: { layout: 'fullscreen', a11y: { test: 'error' } } };
