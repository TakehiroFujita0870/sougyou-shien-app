...
function Composer({ disabled, onSubmit }) {
  ...
  return (
    <form className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-3 shadow-sm" aria-label="Project Kadode AI composer" onSubmit={(event) => { event.preventDefault(); submit(); }}>
      ...
      <div className="flex justify-end border-t border-[var(--color-border-subtle)] pt-2"><Button type="submit" disabled={disabled || !message.trim()} className="min-h-9 rounded-lg px-3" aria-label="送信"><ArrowUp className="size-4" /></Button></div>
    </form>
  );
}

export function ProjectSurface({ state = 'populated', project: projectFixture = demoProjectFixture, adoptedProject, conversationRepository, downloadDocx = downloadFormalPlanDocx }) {
