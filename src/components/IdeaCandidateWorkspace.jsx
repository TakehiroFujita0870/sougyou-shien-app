...
export function IdeaCandidateWorkspace({ repository }) {
  ...
  async function create(event) { event.preventDefault(); const result = await saveCandidate(activeRepository, items, draft); setError(result.error); setItems(result.items); if (!result.error) { setSelected(result.items[0]); setDraft(EMPTY_CANDIDATE); } }
