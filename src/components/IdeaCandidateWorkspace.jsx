...
export function IdeaCandidateWorkspace({ repository, conversationRepository, inputRepository, observer }) {
  ...
  const [items, setItems] = useState([]); const [messages, setMessages] = useState([]); const [input, setInput] = useState(''); const [selected, setSelected] = useState(null); const [error, setError] = useState(''); const [assistPreview, setAssistPreview] = useState(''); const [helpOpen, setHelpOpen] = useState(false);
  const inputChanges = useRef(0); const lastObservedInput = useRef(''); const repeatedEditRecorded = useRef(false); const resumedRecorded = useRef(false);
  useEffect(() => { activeRepository.load().then(setItems).catch(() => setError('候補を読み込めませんでした。')); }, [activeRepository]);
  useEffect(() => { Promise.allSettled([activeConversationRepository.load(), activeInputRepository.load()]).then(([conversationResult, draftResult]) => { const storedMessages = conversationResult.status === 'fulfilled' ? conversationResult.value : []; const draft = draftResult.status === 'fulfilled' ? draftResult.value : ''; setMessages(storedMessages); const latestUserMessage = [...storedMessages].reverse().find((message) => message.role === 'user')?.content.trim(); const restoredDraft = draft.trim() && draft.trim() !== latestUserMessage ? draft : ''; setInput(restoredDraft); if ((storedMessages.length || restoredDraft) && !resumedRecorded.current) { activeObserver.record('idea_message', 'conversation_resumed'); resumedRecorded.current = true; } if (conversationResult.status === 'rejected') setError('会話を読み込めませんでした。'); else if (draftResult.status === 'rejected') setError('入力を読み込めませんでした。'); else if (draft && !restoredDraft) activeInputRepository.save('').catch(() => setError('送信済みの端末内下書きを消去できませんでした。')); }); }, [activeConversationRepository, activeInputRepository, activeObserver]);
  ...
  function updateInput(value) { setInput(value); activeInputRepository.save(value).catch(() => setError('入力を端末内に保存できませんでした。')); if (lastObservedInput.current === value) return; lastObservedInput.current = value; inputChanges.current += 1; if (inputChanges.current >= 3 && !repeatedEditRecorded.current) { activeObserver.record('idea_message', 'repeated_edit'); repeatedEditRecorded.current = true; } }
  async function send(event) { event?.preventDefault(); const content = input.trim(); if (!content) { activeObserver.record('idea_message', 'empty_submit'); setError('発言を入力してください。'); return; } const user = { role: 'user', content }; const next = [...messages, user]; const withReply = [...next, { role: 'assistant', content: nextConversationQuestion(next) }]; try { await activeConversationRepository.save(withReply); } catch { setError('会話を保存できませんでした。再試行してください。'); return; } setMessages(withReply); setInput(''); setAssistPreview(''); try { await activeInputRepository.save(''); setError(''); } catch { setError('発言は保存しましたが、端末内の下書きを消去できませんでした。'); } }
