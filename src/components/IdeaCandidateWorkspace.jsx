...
export function IdeaCandidateWorkspace({ repository, conversationRepository, inputRepository, legacyDraftRepository, observer }) {
  ...
  useEffect(() => { const keyboardDecision = (event) => { if (!selected || (event.key !== 'Enter' && event.key !== ' ')) return; const target = event.target; if (!(target instanceof HTMLButtonElement)) return; const decision = target.textContent === 'プロジェクトに採用' ? 'adopt' : target.textContent === '保留' ? 'hold' : target.textContent === '理由付きで却下' ? 'reject' : ''; if (decision) { event.preventDefault(); decide(decision); } }; window.addEventListener('keydown', keyboardDecision); return () => window.removeEventListener('keydown', keyboardDecision); }, [selected, rejectReason, items, activeRepository]);
