...
foreach ($needle in @('Test-Utf8Preflight', 'HoldSeconds', 'Start-Process', 'WindowStyle Hidden', 'Port conflict', 'systemd-run', 'dots.main:app', 'try {', 'finally {', 'Stop-StartedPreviewResources', 'stateWritten')) { if (-not $start.Contains($needle)) { throw "Start contract missing: $needle" } }
if ($start.Contains('dots.main:create_app')) { throw 'Start contract must use the configured application entry point.' }
