...
MIGRATION = (
    ...
    / "supabase"
    / "migrations"
    / "20260809000100_initial_kadode_schema.sql"
)
...
def test_owner_tables_enable_and_force_rls(table: str) -> None:
    ...
    assert f"create policy {table}_select_own" in SQL
    assert "auth.uid()" in SQL


@pytest.mark.parametrize("table", ("stage_runs", "death_causes", "decision_records"))
...
def test_child_rows_are_bound_to_the_same_owner_as_the_idea() -> None:
    ...
    assert SQL.count("references public.ideas(id, owner_id)") == 3
