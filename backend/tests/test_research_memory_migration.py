...
def test_all_write_policies_check_the_authenticated_owner() -> None:
    for table in TABLES:
        for operation in ("insert", "update"):
            policy = SQL.split(f"create policy {table}_{operation}_own", 1)[1].split(";", 1)[0]
...
def test_document_search_is_owner_derived_and_uses_safe_full_text_indexing() -> None:
    ...
    assert "create function public.search_document_chunks" in SQL
    function = SQL.split("create function public.search_document_chunks", 1)[1].split("$$;", 1)[0]
...
def test_owner_matched_cascades_preserve_existing_decision_records() -> None:
    assert "references public.source_documents(id, owner_id)" in SQL
    assert "references public.document_versions(id, owner_id)" in SQL
    assert "references public.research_runs(id, owner_id)" in SQL
    assert "references public.decision_records(id, owner_id)" in SQL
    assert "on delete cascade" in SQL
    assert "alter table public.decision_records" not in SQL


def test_service_role_is_not_granted_to_browser_facing_roles() -> None:
    assert "grant" in SQL
    assert "service_role" not in SQL
    assert "revoke all on function public.search_document_chunks(text, integer) from public" in SQL
