...
class SpaceKnowledgeRepository:
    ...
    def __init__(self, owner_id: str) -> None:
        if not owner_id.strip():
            raise ValueError("owner principal required")
    ...
    def store_original(self, filename: str, content: bytes, pages: tuple[ExtractedPageInput, ...]) -> StoreResult:
        content_hash = sha256(content).hexdigest()
        existing_id = self._by_hash.get((self._owner_id, content_hash))
        if existing_id is not None:
            return StoreResult(existing_id, content_hash, False)
        original_id = uuid4()
        self._by_hash[(self._owner_id, content_hash)] = original_id
        self._originals[original_id] = _Original(original_id, self._owner_id, filename, content_hash)
    ...
    def add_reference(self, original_id: UUID, command: ReferenceInput) -> _Reference | None:
        original = self._originals.get(original_id)
        if original is None or original.owner_id != self._owner_id or original.deleted:
            return None
        reference = _Reference(uuid4(), self._owner_id, original_id, command.kind, command.source_id, command.locator)
        self._references[reference.id] = reference
        return reference

    def delete_original(self, original_id: UUID) -> bool:
        original = self._originals.get(original_id)
        if original is None or original.owner_id != self._owner_id:
            return False
    ...
    def search(self, query: str) -> tuple[SearchResult, ...]:
        ...
        for reference in self._references.values():
            ...
            if reference.owner_id != self._owner_id or reference.deleted or original is None or original.deleted:
                continue
    ...
    def reference_status(self, original_id: UUID, source_id: str) -> str:
        original = self._originals.get(original_id)
        available = original is not None and not original.deleted and any(reference.owner_id == self._owner_id and reference.original_id == original_id and reference.source_id == source_id and not reference.deleted for reference in self._references.values())
        return "available" if available else "unavailable"

    def original_count(self) -> int:
        return sum(1 for original in self._originals.values() if original.owner_id == self._owner_id and not original.deleted)

    def derived_count(self, original_id: UUID) -> int:
        original = self._originals.get(original_id)
        return len(self._derivatives.get(original_id, ())) if original and original.owner_id == self._owner_id and not original.deleted else 0

    def reference_count(self, original_id: UUID) -> int:
        return sum(1 for reference in self._references.values() if reference.owner_id == self._owner_id and reference.original_id == original_id and not reference.deleted)
