...
- Dots.は本人がPCで使うアプリとして開発する。以後、スマートフォン用画面の新規設計、目視確認、スクリーンショット監査、受け入れ判断、旧モバイル専用テストの実行を行わない。既存の小画面用試作や過去の検査結果は履歴として扱い、新しい作業の根拠にしない。
- UIの標準監査画面は **1280×720 ピクセル、16:9** に固定する。画面を変更したら、この解像度で実画面を開き、見切れ・重なり・可読性・主要操作の到達性を確認する。別解像度を監査対象に追加するには利用者の新たな指示を要する。
- 自動検査では従来機能の回帰防止を維持してよいが、小画面専用の合否を新規UI変更の完了条件にしない。過去の小画面向けテストやStorybook例を増やさない。
